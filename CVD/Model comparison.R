...
for(i in 1:4){
	...
	pred = crossval.cox(x = data[subset, c(metabo.selected3[i], ref[[1]])], y= Surv(data$time[subset], data$event[subset]), theta.fit, theta.predict, ngroup = length(subset))
...
for(i in 1:4){
	print(fits[[i]]$ci[1:3])
	#fits[[5]]$ci[1:3]
	fits.test =  roc.test(fits[[i]], fits.dif_ref[[5]])
	print(fits.test)
