...
chars = characteristics(data.S2[, feature.cont], factor = as.factor(data.S2$inz_mi), d = 2, na.rm = T)
sapply(data.S2[,feature.cont], function(x) wilcox.test(x~data.S2$inz_mi)$p.value)

tapply(data.S2$ctmstati, INDEX = data.S2$inz_mi, function(x) table(x)/length(x))
fisher.test(table(data.S2$my.alkkon, data.S2$inz_mi))
